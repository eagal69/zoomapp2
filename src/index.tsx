  ...
  ['topic', 'name', 'password', 'sessionKey', 'userIdentity'].forEach((field) => {
    if (Object.hasOwn(meetingArgs, field)) {
      try {
        switch(field){
          case 'topic':{
            meetingArgs[field] = getQueryParams("meetingId");
            break
          }
          case 'name':{
            meetingArgs[field] = getQueryParams("un");
            break;
          }
          case 'password':{
            meetingArgs[field] = getQueryParams("psd");
            break;
          }
          case 'userIdentity':{
            meetingArgs[field] = getQueryParams("ui");
            break;
          }
          default:{
...
function getQueryParams(str:string) {
...
if (meetingArgs.web && meetingArgs.web !== '0') {
  ...
  if (meetingArgs.role) {
    meetingArgs.role = parseInt(getQueryParams("ur"), 10);
  } else {
    meetingArgs.role = 0;
  }
}
